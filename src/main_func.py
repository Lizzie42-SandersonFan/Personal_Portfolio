...
def pet_sim_window(root):
    new_window = Toplevel(root)
    new_window.title("Pet Simulator")
    new_window.minsize(250, 250)
    new_window.maxsize(1000,1000)
    ...
    run_btn = tk.Button(new_window, text="Run the code!", command=ps_main)
    run_btn.config(text="This program is running in the terminal, head there!")
    run_btn.pack()
...
def gradebook_window(root):
    ...
    run_btn = tk.Button(new_window, text="Run the code!", command=gb_main)
    run_btn.config(text="This program is running in the terminal, head there!")
    run_btn.pack()
...
def triangle_window(root):
    ...
    run_btn = tk.Button(new_window, text="Run the code!", command=fp_main)
    run_btn.config(text="This program is running in the terminal, head there!")
    run_btn.pack()
...
def movie_window(root):
    ...
    run_btn = tk.Button(new_window, text="Run the code!", command=mv_main)
    run_btn.config(text="This program is running in the terminal, head there!")
    run_btn.pack()
...
def big_main():
    ...
    # Buttons
    btn1 = tk.Button(root, text="Movie Recomender", command=movie_window(root))
    btn1.pack()

    btn2 = tk.Button(root, text="Gradebook Manager", command=gradebook_window(root))
    btn2.pack()

    btn3 = tk.Button(root, text="Pet Simulator", command=pet_sim_window(root))
    btn3.pack()

    btn4 = tk.Button(root, text="Sierpinski Triangle Generator", command=triangle_window(root))
    btn4.pack()
